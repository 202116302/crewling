...
def main():
    ...
    if not os.path.exists(output_file):
        for i in range(1, 5):
            ...
            for i in notion:
                ...
                if date_result.date() >= today:
                    ...
                    number.append(i.select_one("td:nth-of-type(1)").text)
                    # 상세페이지 들어가서 정보 가져오기
                    for x in content:
                        link = x.find('a')['href']
        ...
        df = pd.DataFrame({'번호': number, '제목': title, '신청기간': app, '지원기관': agency, 'URL': site})
        df.to_excel('output\projects.xlsx', index=False)
    ...
    # 2회 이상 실행할 때 (새글 알림)
        ...
        for i in range(1, 5):
            ...
            for i in notion:
                ...
                if date_result.date() >= today:
                    content = i.find_all('td', class_="alignLeft")
                    num = i.select_one("td:nth-of-type(1)").text
                    number.append(num)
                    for q in content:
                        link = q.find('a')['href']
                        notion_pages = requests.get(origin + link)
        ...
        if len(title) > 0:
            ...
            df3.to_excel('output/projects.xlsx', index=False)
        else:
            pass
